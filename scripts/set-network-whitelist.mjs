...
if (!proxyKey) {
  throw new Error(`No proxy key found. Set OCUCLAW_PROXY_KEY env or run the proxy once to generate ${PROXY_KEY_PATH}.`)
}
...
console.log(`Wrote public/ocuclaw-config.json with proxyUrl + proxyKey (${proxyKey.length}-char key baked in)`)
